...
uint16 zclApp_event_loop(uint8 task_id, uint16 events) {
    ...
    if (events & APP_MOTION_DELAY_EVT) {
        ...
        power = 2;
        P1 |=  BV(0);   // power on DD
//        osal_start_timerEx(zclApp_TaskID, APP_MOTION_OFF_EVT, 10000); //adaptive motion
...
//        MicroWait(50);
        ...
        //report
        if (contact){
          zclApp_Magnet_OnOff = 1;
        } else {
          zclApp_Magnet_OnOff = 0;
        }
...
//          MicroWait(50);
          if (power == 0){
            if (!contact) {
//              osal_start_timerEx(zclApp_TaskID, APP_MOTION_OFF_EVT, 100);
            } else {
